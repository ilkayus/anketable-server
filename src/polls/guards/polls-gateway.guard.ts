...
import { WsUnauthorizedException } from 'src/exeptions/websocket-exeptions';
import { SocketWithAuth } from 'src/websocket/socket.types';
import { PollsService } from '../polls.service';
import { PollAuthPayload } from '../types/polls.types';
...
export class GatewayAdminGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      const payload = this.jwtService.verify<PollAuthPayload & { sub: string }>(
        token,
      );
