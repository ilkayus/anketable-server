...
let PollsRepository = PollsRepository_1 = class PollsRepository {
    ...
    async getPoll(pollID) {
        ...
        try {
            ...
            throw new common_1.InternalServerErrorException(`Failed to get poll with: ${pollID}`);
        }
    }
    async startPoll(pollID) {
        ...
        const key = `polls:${pollID}`;
        try {
            await this.redisClient.call('JSON.SET', key, '.hasStarted', JSON.stringify(true));
    ...
    async endPoll(pollID) {
        ...
        const key = `polls:${pollID}`;
        try {
            await this.redisClient.call('JSON.SET', key, '.hasEnded', JSON.stringify(true));
            return this.getPoll(pollID);
        }
        catch (e) {
    ...
    async showResults(pollID, showResults) {
        ...
        const key = `polls:${pollID}`;
        try {
            await this.redisClient.call('JSON.SET', key, '.showResults', JSON.stringify(showResults));
            return this.getPoll(pollID);
        }
        catch (e) {
            this.logger.error(`Failed set showResults for poll: ${pollID}`, e);
            throw new common_1.InternalServerErrorException('The was an error changing showResults value of the poll');
        }
    }
    async addParticipant({ pollID, userID, name, }) {
        ...
        try {
            await this.redisClient.call('JSON.SET', key, participantPath, JSON.stringify(name));
            return this.getPoll(pollID);
        }
        catch (e) {
    ...
    async addNomination({ pollID, nominationID, nomination, }) {
        ...
        try {
            await this.redisClient.call('JSON.SET', key, nominationPath, JSON.stringify(nomination));
            return this.getPoll(pollID);
        }
        catch (e) {
    ...
    async addParticipantRankings({ pollID, userID, rankings, }) {
        ...
        try {
            await this.redisClient.call('JSON.SET', key, rankingsPath, JSON.stringify(rankings));
            return this.getPoll(pollID);
        }
        catch (e) {
    ...
    async addResults(pollID, results) {
        ...
        try {
            await this.redisClient.call('JSON.SET', key, resultsPath, JSON.stringify(results));
            return this.getPoll(pollID);
        }
        catch (e) {
